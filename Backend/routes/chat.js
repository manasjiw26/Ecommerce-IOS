...
const groq = new Groq({
    // If GROQ_API_KEY is not set, we skip Groq and fall back to Ollama/Gemini.
...
router.post('/', async (req, res) => {
    ...
    try {
        try {
            ...
            try {
                const ollamaUrl = process.env.OLLAMA_CLOUD_URL || 'https://ai.amay.fun/api/chat';
                ...
                if (process.env.OLLAMA_CLOUD_AUTH) {
                    headers['X-Custom-Auth'] = process.env.OLLAMA_CLOUD_AUTH;
                }
